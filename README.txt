...
The search is a reletively basic A* based on the given P1 code from 
P2. It prioritized where to search by timecost + heuristic.

The heuristic simply adds inf to any state which produces an
'unnecessary' item. Theorhetically, if given a goal to make 100
benches, it would fail for this reason, so in order to make these
decisions resonably, we disallowed any quantity of a reletively easy to
obtain item higher than the maximum ammount that may be needed for any
...
100 planks, an ammount of wood that would benefit from axes is already
...
For example, 3 axes allowed to exist that may or may not means 8 versions
